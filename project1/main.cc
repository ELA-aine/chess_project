    ...
    while (std::cin >> command) {
       // starting a game 
        if (command == "game") {
            ...
            } else if (whitePlayerType == "computer"){
                whitePlayer = make_unique<ComputerPlayer>(1, true); // constructor 
            }

            if (blackPlayerType == "human") {
                blackPlayer = make_unique<HumanPlayer>(false);
            } else {
                blackPlayer = make_unique<ComputerPlayer>(1, false);
            }
            ...
            if (blackPlayerType == "computer" && !game.getWhite()) {
                game.makeMove("", "", "");
            } else {
