...
Piece* Board::getPiece(const string &coord) const{
    ...
    // cout << coord << " asdasaaaa" << endl;
    int row = 8 - (coord[1] - '0');
    int col = coord[0] - 'a';

    // cout << row << col << " asaaa" << endl;
    ...
    if (row >= 0 && row < 8 && col >= 0 && col < 8) {
        ...
        return nullptr; // If the coordinates are out of bounds, return nullptr
    }
}
...
//     auto lastMove = tempMoveHistory.back();
//     tempMoveHistory.pop_back();

//     Piece *piece = getPiece(to);
//     board[from[1] - '1'][from[0] - 'a'] = std::move(board[to[1] - '1'][to[0] - 'a']);

//     if (!lastMove.capturedPiece.empty()) {
//         char capturedSymbol = lastMove.capturedPiece[0];
//         bool isCapturedWhite = isupper(capturedSymbol);
//         // Create a new Piece object for the captured piece and place it back on the board
//         board[to[1] - '1'][to[0] - 'a'] = std::make_unique<Piece>(capturedSymbol, isCapturedWhite);
//     } else {
//         board[to[1] - '1'][to[0] - 'a'].reset();
//     }
// }
